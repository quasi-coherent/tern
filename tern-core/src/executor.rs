//! This module contains the `MigrationExecutor` trait implemented by third party
//! crates.
...
/// `MigrationExecutor` is the database client interface for migration operations.
pub trait MigrationExecutor: Send + Sync + 'static {
    /// Applies the given [`Query`], either a [`Statement`] in a transaction or
    /// a collection of [`Statements`] executed in order.
    fn apply(&mut self, query: &Query) -> impl Future<Output = TernResult<()>> + Send;
    ...
    /// Reset the history table to point to the given version as the latest.
    ///
