...
/// Operation to resolve the inverse query of a migration.
pub struct RenderInverseQuery<'a, Ctx>(&'a mut Ctx);

impl<'a, Ctx> RenderInverseQuery<'a, Ctx> {
    /// New `RenderQuery` operation.
    pub fn new(ctx: &'a mut Ctx) -> Self {
        Self(ctx)
    }
...
impl<M, Ctx> Operation<&M> for RenderInverseQuery<'_, Ctx>
where
    Ctx: MigrationContext,
    M: Migration<Ctx = Ctx>,
{
    type Output = TernResult<Option<Query>>;

    async fn exec(self, input: &M) -> Self::Output {
        let id = input.migration_id();
        let q = input.revert_query(self.0).await?;
        log::debug!(id:%, query:? = q; "rendered inverse query");
        Ok(q)
    }
}

/// An operation for applying a migration and updating the history table on
/// success.
#[derive(Debug)]
...
/// An operation for applying a down migration and updating the history table on
/// success.
#[derive(Debug)]
    ...
    M: Migration<Ctx = Ctx>,
{
    ...
    async fn exec(self, input: &M) -> Self::Output {
        ...
        let description = id.description();
        let start = Utc::now();
        let Some(q) = RenderInverseQuery(self.ctx).exec(input).await? else {
            return Err(TernError::MissingDown(version));
        };
