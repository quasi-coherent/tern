...
use crate::migration::{AppliedMigration, Migration, MigrationId, MigrationSet};

/// `MigrationContext` is associated with a migration set by providing the
/// "context" needed for any operation during a migration run.
pub trait MigrationContext: Send + Sync {
    ...
    /// Return the [`MigrationSet`] over the provided version range.
    ///
    /// Here, `end` (resp. `start`) defaults to the last (resp. first) version.
    fn new_migration_set(&self, start: Option<i64>, end: Option<i64>) -> MigrationSet<Self>;
    ...
    fn reset_last_applied(&mut self, version: i64) -> BoxFuture<'_, TernResult<()>> {
        Box::pin(async move {
            let history = self.history_table();
            self.executor_mut()
                .reset_last_applied(history, version)
                .await
        })
    }
    ...
    fn all_applied(&mut self) -> BoxFuture<'_, TernResult<Vec<AppliedMigration>>> {
        Box::pin(async move {
            let history = self.history_table();
            let mut applied = self.executor_mut().get_all_applied(history).await?;
    ...
    fn last_applied(&mut self) -> BoxFuture<'_, TernResult<Option<MigrationId>>> {
        Box::pin(async move {
    ...
    fn init_history(&mut self) -> BoxFuture<'_, TernResult<()>> {
        Box::pin(async move {
            let history = self.history_table();
            self.executor_mut()
                .create_history_if_not_exists(history)
                .await
        })
    }
