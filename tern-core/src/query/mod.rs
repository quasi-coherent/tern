...
// Implement `Ord` by hand rather than in the derive macro for extra caution.
//
// A migration is possibly applied by executing a collection of these
// statements.  This determines the order that happens in.  A set of statements
// ran out of order is obviously very likely disastrous.
//
// The `Ord` on a derive macro would change to alphabetical order if the fields
// `sql` and `idx` were swapped, so this guards against that.
...
impl std::cmp::PartialOrd for Statement {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        self.idx.partial_cmp(&other.idx)
    }
}

impl std::cmp::Ord for Statement {
