//! Implementing `tern` for the [`sqlx`] pool types.
//!
//! [`sqlx`]: sqlx::Pool
...
/// Helper trait implemented by each `Db: sqlx::Database` in order to inherit
/// from `sqlx::Pool`.
trait SqlxQueryLib {
...
/// Executor over a generic [`Database`](sqlx::Database).
    ...
    for<'r> (bool,): sqlx::FromRow<'r, <Db as Database>::Row>,
{
    ...
    async fn check_history(&mut self, history: HistoryTable) -> TernResult<()> {
        ...
        let exists: bool =
            sqlx::query_scalar(&sql).fetch_one(self.inner()).await?;
        if exists {
            Ok(())
        } else {
    ...
    async fn reset_last_applied(
        &mut self,
        history: HistoryTable,
        version: i64,
    ) -> TernResult<()> {
        let sql = Db::reset_last_applied_query(history, version);
        sqlx::query::<Db>(&sql).bind(version).execute(self.inner()).await?;
        Ok(())
    }
