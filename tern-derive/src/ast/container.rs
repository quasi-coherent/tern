...
///
/// It's always the same procedure, just a different starting input (represented
/// by `I`).
pub(crate) trait ParseAttr<I>: Default {
...
#[derive(Default)]
pub(crate) struct SkipParseAttr;
...
pub(crate) struct Container<'a, Der, Fld> {
    /// The type having the derive.
    pub(crate) ty: Type<'a>,
    /// Derive attributes.
    pub(crate) attrs: Der,
    /// The fields of the struct.
    pub(crate) fields: Vec<Field<'a, Fld>>,
}
...
{
    pub(crate) fn from_ast(input: &'a syn::DeriveInput) -> Result<Self> {
