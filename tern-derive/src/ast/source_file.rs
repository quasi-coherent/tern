...
impl SourceFile {
    /// For the type deriving `TernApp`, get the `SourceFile` from one entry
    ...
    /// Implementing `Migration` directly for a .sql source.
    pub fn quot_impl_migration_sql(
        &self,
        ident: &syn::Ident,
        content: &syn::LitStr,
        revert_content: Option<&syn::LitStr>,
    ) -> TokenStream {
        let this = &self.this;
        ...
        quote::quote! {
            ...
            impl #this {
                pub(super) fn migration() -> ::tern::migration::MigrationBox<#ident> {
            ...
            impl ::tern::Migration for #this {
                ...
                {
                    let query = ::tern::migration::Query::from_sql(#content);
                ...
                {
                    let revert_raw = #revert_content;
                    let query = revert_raw.map(::tern::migration::Query::from_sql);
                    let fut = ::core::futures::ready(query);
                    ::std::boxed::Box::pin(fut)
                }
            }
    ...
    pub fn quot_impl_migration_rs(&self, ident: &syn::Ident) -> TokenStream {
        ...
        quote::quote! {
            ...
            impl #this {
                pub(super) fn migration() -> ::tern::migration::MigrationBox<<#ident as ::tern::migration::ResolveQuery>::Ctx> {
                    ::tern::migration::MigrationBox::new(#this)
                }
            }
            ...
            impl ::tern::Migration for #this {
                ...
                ) -> ::tern::private::BoxFuture<'a, ::tern::TernResult<::tern::migration::Query>>
                {
                    ::std::boxed::Box::pin(async move {
                        ...
                        <#ident as ::tern::Migration>::query(&migration).await
                    })
                }
                fn revert_query<'a>(
                    &'a self,
                ...
                ) -> ::tern::private::BoxFuture<'a, ::tern::TernResult<Option<::tern::migration::Query>>>
                {
                    ::std::boxed::Box::pin(async move {
                        let migration = <#ident as ::tern::migration::ResolveQuery>::init(ctx).await?;
                        <#ident as ::tern::Migration>::revert_query(&migration).await
                    })
                }
            }

            impl ::tern::Migration for #ident {
                type Ctx = <Self as ::tern::migration::ResolveQuery>::Ctx;
                fn migration_id(&self) -> &::tern::migration::MigrationId {
                    &*__MIGRATION_ID
                }
                fn query<'a>(
                ...
                {
                    <Self as ::tern::migration::ResolveQuery>::resolve_query(ctx)
                }
                fn revert_query<'a>(
                    &'a self,
                    ctx: &'a mut Self::Ctx,
                ...
                {
                    <Self as ::tern::migration::ResolveQuery>::resolve_revert_query(ctx)
                }
            }
        }
    }
