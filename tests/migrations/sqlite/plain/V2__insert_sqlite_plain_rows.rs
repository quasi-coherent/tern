...
use super::SqlitePlain;

#[derive(Migration)]
pub struct SqlitePlainInsertRows {
    rows: i64,
}

impl ResolveMigration for SqlitePlainInsertRows {
    type Ctx = SqlitePlain;
    ...
    async fn resolve(&self, _ctx: &mut Self::Ctx) -> TernResult<Query> {
        ...
        for i in 1..=self.rows {
            builder.push_sql(format!(
                "INSERT INTO tern_sqlite_plain (x, y) VALUES ({i}, 'row{i}');"
            ));
        }
