//! Collections over a set of migrations.
...
impl<Ctx: MigrationContext> UpMigrationSet<Ctx> {
    ...
    /// Return an iterator of migrations over a range of versions.
    pub fn iter_between<'a>(
        self,
        from: Option<i64>,
        to: Option<i64>,
    ) -> Box<dyn Iterator<Item = UpMigration<Ctx>> + Send + 'a>
    where
        Ctx: 'a,
    {
        ...
        let iter = self
            .inner
            .into_iter()
            .skip_while(move |m| m.migration_id().version() < f)
            .take_while(move |m| m.migration_id().version() <= t);
        Box::new(iter)
    }
}
...
impl<'a, Ctx: MigrationContext> UpIterRef<'a, Ctx> {
    fn new(inner: &'a [UpMigration<Ctx>]) -> Self {
        Self { inner, idx: 0 }
    }
}

impl<'a, Ctx: MigrationContext> Iterator for UpIterRef<'a, Ctx> {
    type Item = &'a UpMigration<Ctx>;
...
impl<Ctx: MigrationContext> DownMigrationSet<Ctx> {
    ...
    pub fn iter_between<'a>(
        ...
        from: Option<i64>,
        to: Option<i64>,
    ) -> Box<dyn Iterator<Item = DownMigration<Ctx>> + Send + 'a>
    where
        Ctx: 'a,
    {
        let f = from.unwrap_or(i64::MIN);
        ...
        let iter = self
            .inner
            .into_iter()
            .take_while(move |m| m.migration_id().version() <= f)
            .skip_while(move |m| m.migration_id().version() < t);
        Box::new(iter)
    }
}
...
impl<'a, Ctx: MigrationContext> IntoIterator for &'a DownMigrationSet<Ctx> {
