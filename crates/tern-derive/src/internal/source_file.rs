...
impl SourceFile {
    ...
    /// Assumes that the user's type `ident` implements `ResolveMigration`, then
    /// sets `#module::#this::migration()` to `Up/DownMigration::new_pending`.
    pub fn quot_impl_migration_rs(&self, ident: &syn::Ident) -> TokenStream {
        ...
        quote::quote! {
            ...
            impl #this {
                pub(super) fn migration() -> #mig<<#ident as ::tern::ResolveMigration>::Ctx> {
                    #mig::new_pending::<#ident>(#mid)
                }
            }
        }
    }
