...
/// Collecting the whole token stream for `Tern`.
pub fn expand_impl_tern(input: &syn::DeriveInput) -> Result<TokenStream> {
    let container = TernAppContainer::new(input)?;
    let output = container.quot_impl_tern_app()?;
    Ok(output)
}

// Token stream for expand_impl_tern.
type TernAppContainer<'a> = Container<'a, TernAppDeriveAttr, TernAppFieldAttr>;
...
impl<'a> TernAppContainer<'a> {
    ...
    fn quot_impl_tern_app(&self) -> Result<TokenStream> {
        let ident = &self.ty.ident;
        let src_mods = self.attrs.source_mods(ident)?;
        let impl_tern = src_mods.quot_impl_tern_mods(ident);
...
impl TernAppDeriveAttr {
    ...
    fn quot_history_table_fn(&self) -> TokenStream {
        let quot_tbl = self
            .table
            .as_ref()
            .map(|t| quote::quote! { ::tern::migration::HistoryTable::new(#t) })
            .unwrap_or(
                quote::quote! { ::tern::migration::HistoryTable::default() },
            );
        let quot_hist_new = match self.schema.as_ref() {
            Some(s) => quote::quote! { #quot_tbl.with_schema(#s) },
            _ => quot_tbl,
        };

        quote::quote! {
            fn history_table(&self) -> ::tern::migration::HistoryTable {
                #quot_hist_new
            }
        }
    }
